...
while True:
    ...
    fmask = cv2.erode(fmask, cv2.getStructuringElement(
        cv2.MORPH_ERODE, (2, 2)), iterations=2)
    mask1 = cv2.morphologyEx(fmask, cv2.MORPH_CLOSE,
                             cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (4, 4)))
    mask1 = cv2.erode(mask1, cv2.getStructuringElement(
        cv2.MORPH_ERODE, (2, 2)), iterations=2)
    ...
    try:
        if cv2.contourArea(my_con) > 90:
            ...
            cv2.line(roi, leftmost,
                     (topmost[0], bottommost[1]-80), (0, 242, 225), 2)
